...
AdjacencyMatrix* createAdjacencyMatrix(int defaultEdgeValue)
{
    ...
    AdjacencyMatrix* newMatrix = (AdjacencyMatrix*)myMalloc(sizeof(AdjacencyMatrix));
    ...
    // Initialize all elements of the matrix to the defaultEdgeValue
    for (int i = 0; i < NUMBER_OF_VERTICES; i++) {
        for (int j = 0; j < NUMBER_OF_VERTICES; j++) {
            newMatrix->matrix[i][j] = defaultEdgeValue;
        }
    }
